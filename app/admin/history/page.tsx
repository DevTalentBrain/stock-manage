...
export default function HistoryPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      await Promise.all([fetchOrders(), fetchDeliveries()]);
      setLoading(false);
    }
    load();
  }, []);
  ...
  const filteredDeliveries = deliveries.filter((d) => {
    const search = searchTerm.toLowerCase();
    const origin = (d.get("origin") || "").toLowerCase();
    const destination = (d.get("destination") || "").toLowerCase();
          ...
          <div className="p-8 border-b border-gray-100 bg-gray-50/30 flex-shrink-0">
            <div className="relative max-w-xl">
              ...
              <input
                ...
                placeholder={
                  activeTab === "orders"
                    ? "Search by Stockholder, email, or city..."
                    : "Search by route or item name..."
                }
                value={searchTerm}
          ...
          <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
            ...
            {!loading && (
              <>
                ...
                {activeTab === "cargo" && filteredDeliveries.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-20">
                    <p className="text-gray-400 font-black uppercase text-xs tracking-widest">
